...
- [ ] **Extend `site/src/site.config.ts`** (AC1) — Add `contact` (and export type). Keep `as const`:
  ```ts
...
- [ ] **Update `SiteHeader.astro`** (AC2, AC5) — Render the Contact nav item from `siteConfig.nav`. Adjust `isCurrent()` so **`mailto:`** (and `http(s):`) links **never** receive `aria-current="page"`:
  ```ts
...
- [ ] **Create `site/src/components/nav/SiteFooter.astro`** (AC3, AC5) — Semantic footer landmark:
  ```astro
  ---
...
- [ ] **Wire footer into `BaseLayout.astro`** (AC3) — Same pattern as header slot fallback:
  ```astro
...
- [ ] **Refactor `HomeContact.astro`** (AC4) — Import `siteConfig`; set `href={`mailto:${siteConfig.contact.email}`}` and visible text from `siteConfig.contact.email`. Keep legacy layout/copy (“Say hello”, social row) unchanged.

- [ ] **FR20 verification matrix** (AC2, AC4) — After build, confirm `mailto:` appears in:
...
- [ ] **Document contact policy in `site/README.md`** (AC5) — Short §Contact (3–5 lines):
...
- [ ] **Validation gates** (AC6) — `npm run check`, `npm run build`, `npm run test:schema` → exit 0.

- [ ] **Manual smoke** — On `/cv`: Contact visible in header without scrolling; keyboard activates mailto; footer link focus ring visible; no horizontal scroll from footer on 320px.

- [ ] **Do not** add `/contact` route, contact form, API, or client JS.
