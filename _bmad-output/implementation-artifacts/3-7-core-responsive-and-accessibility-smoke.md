...
- [ ] **Prerequisite gate** — Confirm Stories **3.5** (`404.astro` + `dist/404.html`) and **3.6** (footer + nav Contact) are **done** before executing smoke. If either is still `ready-for-dev`, implement or complete them first — this story validates the **whole** Epic 3 chrome set.

- [ ] **Create `docs/core-pages-smoke-checklist.md`** (AC1, AC3) — Use the template in [Checklist template](#checklist-template) below. Keep it **manual** (checkboxes / PASS-FAIL tables). Epics allow “automated **or** documented manual” — no new test framework required.

- [ ] **Document standard breakpoints** (AC1, AC6) — Add to checklist header **and** `site/README.md` §Responsive smoke:
...
- [ ] **Run responsive matrix** (AC2) — Routes (built preview):
...
- [ ] **Run accessibility smoke** (AC4) — Consolidate checks from Stories 3.1–3.6 into one pass per route:
...
- [ ] **Fix regressions only** (AC2) — Allowed files: layout/components/pages scoped CSS for **`site/src/**`** touched by Epic 3. **No** redesign, new features, or blog routes. Typical fixes:
...
- [ ] **Sign off checklist** (AC3) — Fill footer: tester, date, commit SHA, all PASS.

- [ ] **Update `site/README.md`** (AC6) — Add §Responsive smoke linking `../docs/core-pages-smoke-checklist.md` and listing breakpoint tokens.

- [ ] **Validation gates** (AC5) — `npm run check`, `npm run build`, `npm run test:schema` after fixes.

- [ ] **Do not** add Playwright/Cypress, Lighthouse CI, or axe-core unless product owner expands scope (out of epic AC).
